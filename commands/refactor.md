...
After identifying the refactoring scope and before executing, launch two agents **simultaneously in a single message**:

1. **Refactor agent**: Implements the code changes according to the plan
2. **Test agent**: Simultaneously identifies which existing tests need updating and drafts the required test changes to cover the refactored code

Wait for both agents to return. Merge the code changes and test changes together before committing. Validate the merged result runs cleanly before proceeding.
