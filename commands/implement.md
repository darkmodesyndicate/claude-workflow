...
Before writing any code or creating the plan, launch a **Plan subagent** to explore the codebase:

```
Launch a Plan subagent with the following task:
- Explore the codebase to find existing patterns that match this feature
- Identify all files that will need to be created or modified
- Check for any existing implementations or utilities that should be reused
- Check for potential conflicts with existing code (naming, routes, events)
- Identify the correct architectural layer for each new piece (controller, service, repository, Livewire component, etc.)
- Return: a proposed list of files to create/modify, in implementation order
```

Present the exploration findings and proposed implementation order to the user. Pause for confirmation before proceeding to plan creation.

## Phase 3: Strategic Planning

Based on the subagent's exploration and user confirmation, create an implementation plan:
